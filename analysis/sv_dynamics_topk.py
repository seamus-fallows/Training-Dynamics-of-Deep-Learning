"""Plot SV dynamics for top-k truncated power-law teacher.

One figure per (k, gamma_config, partial_product, regime).
Each figure: (3 hidden_dims × 2) rows × 4 columns
  - Row pairs per hidden_dim: loss, singular values
  - Columns: model seeds 0–3

Usage:
    python -m analysis.sv_dynamics_topk
"""
...
GAMMA_CONFIGS = [(0.75, 7000), (1.0, 8000), (1.5, 26000)]
GAMMA_NAMES = {0.75: "NTK", 1.0: "Mean-Field", 1.5: "Saddle-to-Saddle"}

out_dir = Path("figures/sv_dynamics_topk")

# ── Data loading ────────────────────────────────────────────────────────

df_off = load_sweep(Path("outputs/sv_dynamics_topk/offline"))
df_on = load_sweep(Path("outputs/sv_dynamics_topk/online"))

hidden_dims = sorted(df_off["model.hidden_dim"].unique().to_list())
k_values = sorted(df_off["data.params.k"].unique().to_list())
...
def get_row(df, gamma, max_steps, hd, seed, batch_size, k):
...
def load_run(df, gamma, max_steps, hd, seed, batch_size, k):
    """Load steps, loss, and SV data for a single run."""
    row = get_row(df, gamma, max_steps, hd, seed, batch_size, k)
    data = {
...
for k in k_values:
    for gamma, max_steps in GAMMA_CONFIGS:
        ...
        for hd in hidden_dims:
            ...
            for seed in seeds:
                all_runs[hd][seed] = {
                    "off_gd": load_run(df_off, gamma, max_steps, hd, seed, off_gd_bs, k),
                    "off_sgd": load_run(df_off, gamma, max_steps, hd, seed, off_sgd_bs, k),
                    "on_gd": load_run(df_on, gamma, max_steps, hd, seed, on_gd_bs, k),
                    "on_sgd": load_run(df_on, gamma, max_steps, hd, seed, on_sgd_bs, k),
                }
        ...
        for regime_key, regime_label, _, _, _ in regimes:
            ...
            regime_dir = out_dir / regime_key / f"k{k}" / f"g{gamma}"
            ...
            for i, j in partial_products:
                ...
                for hd_idx, hd in enumerate(hidden_dims):
                    ...
                    for col, seed in enumerate(seeds):
                        ...
                        for sv_idx in range(n_plot):
                            line, = ax.plot(r[gd_key]["steps"], r[gd_key][pp_key][:, sv_idx], label=f"\u03c3_{sv_idx}")
                            ax.plot(r[sgd_key]["steps"], r[sgd_key][pp_key][:, sv_idx], ls="--", color=line.get_color())
                        if col == len(seeds) - 1:
                ...
                fig.suptitle(
                    ...
                    f"{regime_label} | {gamma_name} initialisation (\u03b3 = {gamma}) | teacher rank = {k}",
        ...
        print(f"k={k}, \u03b3={gamma}: saved {2 * len(partial_products)} figures (offline + online)")
