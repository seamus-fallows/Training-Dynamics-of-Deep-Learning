"""
Batch Distance (Power-Law Teacher) Analysis

Plots overlaid curves from a comparative sweep where B=500 (model A) and
B=1 (model B) are trained on a power-law teacher matrix in online mode with
different batch seeds.
...
Usage:
    python analysis/batch_distance_power_law.py
    python analysis/batch_distance_power_law.py --input outputs/batch_distance_power_law
"""
...
DEFAULT_INPUT = Path("outputs/batch_distance_power_law")
DEFAULT_OUTPUT = Path("figures/batch_distance_power_law")
...
def plot_distances(
    ...
    for col_idx, (gamma, df) in enumerate(groups):
        ...
        if has_loss:
            ...
            ax.plot(steps, loss_a[0], color="C0", linewidth=1.5, label="B=500")
            for j in range(len(loss_b)):
                ax.plot(steps, loss_b[j], color="C1", alpha=0.12, linewidth=0.4)
            ax.plot(steps, loss_b.mean(axis=0), color="black", linewidth=1.5, linestyle="--", label="B=1 mean")
            ax.set_yscale("log")
            if col_idx == 0:
        ...
        ax.set_yscale("log")
        if col_idx == 0:
            ax.set_ylabel(r"$\|\theta_{B=500} - \theta_{B=1}\|$")
        if not has_loss:
            ax.set_title(_gamma_title(gamma))
        ax.legend(fontsize=7)
        row += 1
        ...
        if has_cosine:
            ...
            if col_idx == 0:
                ax.set_ylabel(r"$\cos(\theta_{B=500}, \theta_{B=1})$")
            ax.legend(fontsize=7)
            row += 1
        ...
        if n_layers > 0:
            ...
            ax.set_yscale("log")
            if col_idx == 0:
                ax.set_ylabel(r"$\|W_i^{B=500} - W_i^{B=1}\|_F$")
    ...
    fig.suptitle(
        f"B=500 vs B=1 ({n_total} batch seeds) | online power-law teacher",
...
def plot_norms(
    ...
    for col_idx, (gamma, df) in enumerate(groups):
        ...
        for i in range(n_layers):
            ...
            # Model A norms (B=500 — deterministic given fixed batch_seed)
            col_a = f"layer_norm_{i}_a"
            if col_a in df.columns:
                curves_a = _extract(df, col_a)
                ax.plot(steps, curves_a[0], color="C0", linewidth=1.5, label="B=500")

            # Model B norms (B=1 — vary across batch seeds)
            ...
            if col_b in df.columns:
                ...
                ax.plot(
                    ...
                    color="black", linewidth=1.5, linestyle="--", label="B=1 mean",
                )
    ...
    n_total = sum(len(df) for _, df in groups)
    fig.suptitle(
        f"Layer norms: B=500 vs B=1 ({n_total} batch seeds) | online power-law teacher",
        fontsize=13, fontweight="bold",
    )
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot B=500 vs B=1 distance curves from online power-law teacher comparative sweep",
    )
